...
for ($id = 1 ; $id <= $idempleadoarray[0]; $id++) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        //ID DE EMPLEADO
        //$sheet->setCellValue('A'.$filacon, "id EMP : " .  $row['idempleado_prod']);
        $idemp[$id] = $row['idempleado_prod'];
        //echo 'id empleado ' . $row['idempleado_prod'];
        //$sheet->setCellValue('A'.$filacon, "idemp_prod : " .  $row['idempleado_prod']);
        $nombreemp[$id] = $row['nombreempleado'];
        //$sheet->setCellValue('B'.$filacon, "nombre empleado : " . $row["MAX(idempleado_prod)"]);
        $apellidoemp[$id] = $row['apellidoempleado'];
       // $sheet->setCellValue('B'.$filacon, "" . $row['nombreempleado'] . " " . $row['apellidoempleado'] );
